    ...
    <div className="group bg-white rounded-2xl sm:rounded-3xl overflow-hidden border border-slate-100 shadow-xs hover:shadow-xl transition-all duration-300 flex flex-col h-full card-hover-effect">
      ...
      <div className="relative aspect-[16/10] w-full overflow-hidden bg-slate-100">
        ...
        <div className="absolute top-2.5 left-2.5 sm:top-3 sm:left-3 flex flex-wrap items-center gap-1.5 z-10">
          {business.category && (
            <span className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 sm:py-1 rounded-full bg-white/90 backdrop-blur-md text-slate-800 shadow-xs">
              {business.category.name}
            </span>
          )}
          {business.is_featured && (
            <span className="text-[10px] font-extrabold uppercase tracking-wider px-2.5 py-0.5 sm:py-1 rounded-full bg-gradient-to-r from-amber-400 to-orange-500 text-slate-950 shadow-xs flex items-center gap-1">
              <span>⭐ Featured</span>
            </span>
          )}
        ...
        <button
          ...
          className={`absolute top-2.5 right-2.5 sm:top-3 sm:right-3 z-10 p-2 rounded-full backdrop-blur-md transition-all cursor-pointer ${
            favorited
        ...
        {activePromo && (
          <div className="absolute bottom-2.5 left-2.5 sm:bottom-3 sm:left-3 z-10">
            <span className="inline-flex items-center gap-1 text-[10px] font-extrabold px-2.5 py-0.5 sm:py-1 rounded-full bg-red-600 text-white shadow-md">
      ...
      <div className="p-4 sm:p-5 flex-1 flex flex-col justify-between">
        <div>
          ...
          <div className="flex items-center justify-between gap-2 mb-1.5 sm:mb-2">
            ...
            <span className="text-[11px] sm:text-xs font-black tracking-wider text-slate-700 bg-slate-100 px-2 py-0.5 rounded-md">
          ...
          <Link to={`/businesses/${business.slug}`} className="block group-hover:text-emerald-700 transition-colors">
            <h3 className="font-bold text-base sm:text-lg text-slate-900 leading-snug group-hover:text-emerald-700 transition-colors line-clamp-1">
              {business.name}
            </h3>
            {business.khmer_name && (
              <p className="text-xs font-khmer text-slate-400 mt-0.5 line-clamp-1">
                {business.khmer_name}
              </p>
            )}
          </Link>

          <p className="text-xs text-slate-500 mt-1.5 sm:mt-2 line-clamp-2 leading-relaxed">
            {business.short_description || business.description}
          </p>

          <p className="text-[10px] sm:text-[11px] text-slate-400 mt-2 sm:mt-3 flex items-center gap-1 truncate">
        ...
        {/* Footer actions */}
        <div className="pt-3 sm:pt-4 mt-3 sm:mt-4 border-t border-slate-50 flex items-center justify-between gap-2">
          <span className="text-[10px] sm:text-[11px] text-slate-500 font-medium truncate max-w-[130px] sm:max-w-[160px]">
            {business.opening_hours || 'Open Daily'}
          </span>

          <Link
            to={`/businesses/${business.slug}`}
            className="inline-flex items-center gap-1 text-xs font-bold text-emerald-700 hover:text-emerald-800 group/btn shrink-0"
          >
