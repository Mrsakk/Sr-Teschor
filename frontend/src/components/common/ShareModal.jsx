...
export default function ShareModal({ isOpen, onClose, title, url, description }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-md rounded-xl shadow-md border border-slate-100 overflow-hidden p-6 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-3">
          {shareOptions.map((opt) => (
            <button
              key={opt.name}
              onClick={opt.action}
              className={`flex flex-col items-center justify-center gap-2 p-3 rounded-xl transition-all hover:scale-105 shadow-sm ${opt.color}`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-bold transition-all ${
                copied
                  ...
                  : 'bg-orange-500 hover:bg-orange-600 text-white shadow-sm shadow-sm'
              }`}
            >
              {copied ? (
                <>
        ...
        {typeof navigator !== 'undefined' && navigator.share && (
          <button
            ...
            className="w-full py-3 rounded-xl border border-slate-200 text-slate-700 hover:bg-slate-50 font-bold text-xs flex items-center justify-center gap-2"
          >
