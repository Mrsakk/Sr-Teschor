    ...
    <div className="group bg-white rounded-xl sm:rounded-xl overflow-hidden border border-slate-100 shadow-xs hover:shadow-sm transition-all duration-300 flex flex-col h-full card-hover-effect">
      ...
      <div className="relative aspect-[4/3] w-full overflow-hidden bg-slate-100">
        ...
        <div className="absolute bottom-2.5 left-2.5 right-2.5 sm:bottom-3 sm:left-3 sm:right-3 text-white">
          <p className="text-[10px] sm:text-[11px] font-medium text-slate-200 flex items-center gap-1 truncate">
      ...
      <div className="p-4 sm:p-5 flex-1 flex flex-col justify-between">
        <div>
          ...
          <div className="flex items-center justify-between gap-2 mb-1.5 sm:mb-2">
            ...
            <span className="text-[11px] sm:text-xs font-extrabold text-orange-600 bg-orange-50 px-2 py-0.5 rounded-full shrink-0">
          ...
          <Link to={`/destinations/${destination.slug}`} className="block group-hover:text-orange-600 transition-colors">
            <h3 className="font-bold text-base sm:text-lg text-slate-900 leading-snug group-hover:text-orange-600 transition-colors line-clamp-1">
              {destination.name}
            </h3>
        ...
        <div className="pt-3 sm:pt-4 mt-3 sm:mt-4 border-t border-slate-50 flex items-center justify-between gap-2">
          <div className="text-[10px] sm:text-[11px] text-slate-400 flex items-center gap-1 min-w-0">
            <Clock className="w-3 h-3 text-slate-400 shrink-0" />
            <span className="truncate">{destination.best_time ? destination.best_time.split('&')[0] : 'Open Daily'}</span>
          </div>

          <Link
            to={`/destinations/${destination.slug}`}
            className="inline-flex items-center gap-1 text-xs font-bold text-orange-600 hover:text-orange-700 group/btn shrink-0"
          >
            <span>Explore</span>
