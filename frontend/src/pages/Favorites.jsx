...
export default function Favorites() {
  ...
  const destinations = favData?.destinations || [];
  const businesses = favData?.businesses || [];
  const loading = isLoading && !favData;
    ...
    return (
      <div className="pt-24 sm:pt-32 pb-24 max-w-md mx-auto px-4 text-center space-y-4">
        ...
        <p className="text-xs sm:text-sm text-slate-500 leading-relaxed">
          Sign in to save temples, hotels, and restaurants you love for your next trip.
        </p>
        <div className="pt-2">
          <Link
            to="/login"
  ...
  return (
    <div className="pt-24 sm:pt-28 pb-28 sm:pb-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6 sm:space-y-8">
      
      {/* Header & Tabs */}
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 pb-4 sm:pb-6 border-b border-slate-200">
        <div>
          <span className="text-[11px] sm:text-xs font-bold uppercase tracking-wider text-rose-600 flex items-center gap-1.5">
            <Heart className="w-3.5 h-3.5 fill-rose-600" /> Saved Collection
          </span>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 font-heading mt-1">
            Favorite Places ({totalCount})
          </h1>
          <p className="text-xs text-slate-500 mt-0.5">
            Temples, hotels, and restaurants you've saved
          </p>
        </div>

        {/* Responsive Tabs */}
        <div className="grid grid-cols-3 sm:flex items-center gap-1 bg-slate-100 p-1 rounded-xl text-[11px] sm:text-xs font-bold w-full sm:w-auto border border-slate-200/80">
          <button
            onClick={() => setActiveTab('all')}
            className={`py-2 px-3 sm:px-4 rounded-lg transition-all text-center cursor-pointer ${
      ...
      {!loading && totalCount === 0 && (
        <div className="bg-white rounded-2xl p-8 sm:p-16 text-center border border-slate-200 max-w-lg mx-auto space-y-4 shadow-xs">
          <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-rose-50 text-rose-600 flex items-center justify-center mx-auto border border-rose-100">
            <Heart className="w-7 h-7 sm:w-8 sm:h-8 fill-rose-600/20" />
          </div>
          <h3 className="text-base sm:text-lg font-bold text-slate-900 font-heading">
            You haven't saved any places yet
          </h3>
          <p className="text-xs text-slate-500 max-w-sm mx-auto leading-relaxed">
            Click the heart button on destination or business cards to save them to your favorites.
          </p>
          <div className="pt-2">
            <Link
              to="/destinations"
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-orange-600 hover:bg-orange-700 text-white font-bold text-xs transition-colors shadow-xs"
            >
              <Compass className="w-4 h-4" /> <span>Find Destinations</span>
            </Link>
          </div>
        </div>
      )}
      ...
      {!loading && (activeTab === 'all' || activeTab === 'destinations') && destinations.length > 0 && (
        <div className="space-y-4">
          <h2 className="text-base sm:text-lg font-extrabold text-slate-900 flex items-center gap-2 font-heading">
            <Compass className="w-4 h-4 sm:w-5 sm:h-5 text-orange-600" />
