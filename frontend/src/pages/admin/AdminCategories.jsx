  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-white tracking-tight">Category Classification</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {isLoading ? (
          ...
          categories.map((cat) => (
            <div
              key={cat.id}
              className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-xl hover:border-slate-700 transition-all group flex flex-col justify-between relative"
            >
              <div className="h-28 relative overflow-hidden">
                ...
                <div className="absolute top-3 right-3 flex items-center gap-1">
                  <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase bg-slate-950/80 backdrop-blur-sm text-emerald-400 border border-emerald-500/30">
                    {cat.type}
                  </span>
                </div>
              </div>
              ...
              <div className="p-5 flex-1 flex flex-col justify-between">
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span className="text-xs font-mono text-slate-500">Order #{cat.display_order}</span>
                  </div>
                ...
                <div className="pt-4 mt-3 border-t border-slate-800/80 flex items-center justify-between">
                  <div className="flex items-center gap-2 text-xs text-slate-400">
                    <span className="font-semibold text-emerald-400">{cat.destinations_count || 0}</span> Attractions •{' '}
                    <span className="font-semibold text-sky-400">{cat.businesses_count || 0}</span> Businesses
                  </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5">
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <label className="text-[10px] text-emerald-400 hover:text-emerald-300 cursor-pointer flex items-center gap-1 font-bold">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-slate-300 mb-1">Applicable Scope</label>
                  <select
                    value={formData.type}
