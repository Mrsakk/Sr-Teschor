            ...
            <div className="flex items-center gap-4 pb-5 border-b border-slate-800">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-[10px] bg-emerald-500/20 text-emerald-400 font-semibold px-2 py-0.5 rounded-full capitalize">
                    {selectedUser.role}
                  </span>
                ...
                {selectedUser.bookings?.length > 0 ? (
                  <div className="space-y-2">
                    {selectedUser.bookings.map((b) => (
                      <div key={b.id} className="p-3 rounded-xl bg-slate-800/60 border border-slate-800 text-xs flex items-center justify-between">
                        <div>
                          <p className="font-semibold text-white">#{b.booking_reference} — {b.business?.name}</p>
                          <p className="text-slate-400">{b.service?.name} • ${b.total_amount}</p>
                        </div>
                        <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-slate-700 text-slate-300 capitalize">
                          {b.status}
                        </span>
                      </div>
                    ))}
                  ...
                  <p className="text-xs text-slate-500">No booking reservations recorded.</p>
                )}
              </div>
                ...
                {selectedUser.reviews?.length > 0 ? (
                  <div className="space-y-2">
                    {selectedUser.reviews.map((r) => (
                      <div key={r.id} className="p-3 rounded-xl bg-slate-800/60 border border-slate-800 text-xs">
                        <div className="flex items-center justify-between mb-1">
                          <span className="font-semibold text-amber-400">★ {r.rating}/5</span>
                  ...
                  <p className="text-xs text-slate-500">No submitted reviews.</p>
                )}
              </div>
            </div>
          </div>
