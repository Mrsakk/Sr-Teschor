    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="px-3.5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-semibold shadow-sm shadow-emerald-600/20 flex items-center gap-1.5 transition-all cursor-pointer"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {isLoading ? (
          ...
          <div className="col-span-full text-center py-16 bg-white/60 rounded-xl border border-slate-200 text-slate-400 space-y-3">
            ...
            <p className="text-xs text-slate-400">Create a new ad placement or let businesses sponsor placements.</p>
          </div>
        ) : (
          ads.map((ad) => (
            <div
            ...
            >
              <div className="h-32 relative overflow-hidden">
                ...
                <div className="absolute top-3 left-3 flex items-center gap-1.5">
                  <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase bg-slate-50/80 text-amber-300 border border-amber-500/30">
                    {String(ad.placement || 'hero_banner').replace('_', ' ')}
                  </span>
                  {ad.status === 'active' ? (
                    ad.is_expiring_soon ? (
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase bg-orange-500/80 text-slate-900 animate-pulse">
                    ...
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase bg-rose-500/80 text-slate-900">
                      {ad.status || 'Expired'}
                    </span>
                  )}
                </div>
                <div className="absolute top-3 right-3 font-bold text-xs text-slate-900 bg-slate-50/80 px-2 py-0.5 rounded-lg border border-slate-200">
                  ${ad.price || '0.00'}
                </div>
              </div>
              ...
              <div className="p-5 flex-1 flex flex-col justify-between">
                <div>
                  <h3 className="text-sm font-bold text-slate-900 mb-1">{ad.title || 'Untitled Campaign'}</h3>
                  <p className="text-xs text-emerald-600 font-medium mb-3">{ad.business?.name || 'Partner Business'}</p>

                  <div className="grid grid-cols-3 gap-2 bg-transparent hover:bg-slate-50 p-2.5 rounded-xl border border-slate-200 text-xs">
                    ...
                    <div className="flex items-center gap-1.5 text-slate-700">
                      <span className="text-amber-600 font-bold">CTR: {ad.ctr || 0}%</span>
                    </div>
                  </div>
                </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-50/80 backdrop-blur-sm">
          <div className="bg-white border border-slate-200 rounded-xl max-w-md w-full p-6 shadow-md relative text-left">
            ...
            <h3 className="text-base font-bold text-slate-900 mb-4">Create Sponsored Placement</h3>

            <form onSubmit={handleCreate} className="space-y-3.5">
              <div>
                <label className="block text-xs font-medium text-slate-700 mb-1">Partner Business</label>
                <select
              ...
              <div>
                <label className="block text-xs font-medium text-slate-700 mb-1">Campaign Headline</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-xs font-medium text-slate-700 mb-1">Banner Image URL</label>
                <input
                  type="url"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-slate-700 mb-1">Placement</label>
                  <select
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-700 mb-1">Price ($ USD)</label>
                  <input
                    type="number"
