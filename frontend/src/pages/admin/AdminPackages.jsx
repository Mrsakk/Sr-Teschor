    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white text-xs font-extrabold shadow-sm shadow-orange-500/20 flex items-center gap-2 transition-all cursor-pointer"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-white border border-slate-200 p-4 rounded-xl">
          ...
          <span className="text-xl font-black text-slate-900 mt-1 block">{stats.total_packages}</span>
        </div>
        <div className="bg-white border border-slate-200 p-4 rounded-xl">
          <span className="text-[11px] text-slate-400 font-bold block">Active Online</span>
          <span className="text-xl font-black text-emerald-600 mt-1 block">{stats.active_packages}</span>
        </div>
        <div className="bg-white border border-slate-200 p-4 rounded-xl">
          <span className="text-[11px] text-slate-400 font-bold block">Average Price</span>
          <span className="text-xl font-black text-amber-600 mt-1 block">${stats.avg_price}</span>
        </div>
        <div className="bg-white border border-slate-200 p-4 rounded-xl">
          ...
          <span className="text-xl font-black text-sky-600 mt-1 flex items-center gap-1">
            <Star className="w-4 h-4 fill-amber-400 text-amber-600 inline" />
      ...
      {/* 3. Search & Filter Bar */}
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-white/60 p-3 rounded-xl border border-slate-200">
        <div className="relative w-full sm:w-80">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-white border border-slate-200 rounded-xl text-xs text-slate-900 placeholder-slate-500 focus:outline-none focus:border-orange-500"
          />
        </div>
        ...
        <div className="flex items-center gap-2 self-end sm:self-auto text-xs">
          <button
            ...
            className={`px-3 py-1.5 rounded-xl font-bold transition-all cursor-pointer ${
              statusFilter === 'all' ? 'bg-orange-600 text-white' : 'text-slate-400 hover:text-slate-900'
            }`}
          >
            All
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-xl font-bold transition-all cursor-pointer ${
              statusFilter === 'active' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:text-slate-900'
            }`}
          >
            Active
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-xl font-bold transition-all cursor-pointer ${
              statusFilter === 'inactive' ? 'bg-rose-600 text-white' : 'text-slate-400 hover:text-slate-900'
            }`}
          >
            Inactive
          </button>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 bg-white/60 rounded-xl border border-slate-200 text-slate-400 space-y-3">
          <Package className="w-10 h-10 text-slate-600 mx-auto" />
          <p className="text-sm font-semibold">No travel packages found.</p>
          <p className="text-xs text-slate-400">Click "New Travel Package" to create your first tour package.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {packages.map((pkg) => {
            ...
            return (
              <div
                key={pkg.id}
                className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm flex flex-col justify-between group hover:border-slate-200 transition-colors"
              >
                <div>
                  {/* Image Banner */}
                  <div className="h-44 relative overflow-hidden bg-slate-50">
                    ...
                    <div className="absolute top-3 left-3 flex items-center gap-1.5">
                      <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase bg-slate-50/80 backdrop-blur-md text-amber-300 border border-amber-500/30 flex items-center gap-1">
                    ...
                    <div className="absolute bottom-3 left-3 right-3 flex items-end justify-between text-slate-900">
                      <div>
                        <span className="text-[10px] text-slate-700 block font-semibold">Selling Price</span>
                        <span className="text-xl font-black text-amber-300">${Number(pkg.selling_price).toFixed(2)}</span>
                      </div>
                      <div className="flex items-center gap-1 bg-slate-50/80 px-2 py-0.5 rounded-lg border border-slate-200 text-xs font-bold text-amber-600">
                  ...
                  <div className="p-5 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2 bg-slate-50/80 p-2.5 rounded-xl border border-slate-200 text-[11px]">
                      <div>
                        <span className="text-slate-400 block text-[10px]">Provider Cost:</span>
                        <span className="font-bold text-slate-700">${Number(pkg.provider_cost || 0).toFixed(2)}</span>
                      </div>
                      <div>
                        <span className="text-slate-400 block text-[10px]">Platform Profit:</span>
                        <span className="font-bold text-emerald-600">+${margin.toFixed(2)}</span>
                      </div>
                    </div>
                    ...
                    {incList.length > 0 && (
                      <div className="space-y-1.5 pt-2 border-t border-slate-200/80">
                        <span className="text-[10px] uppercase font-bold text-slate-400 block">Includes:</span>
                ...
                <div className="p-4 bg-slate-50/60 border-t border-slate-200 flex items-center justify-between gap-2">
                  <span className="text-[10px] text-slate-400">ID #{pkg.id}</span>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-50/80 backdrop-blur-md overflow-y-auto">
          <div className="bg-white border border-slate-200 max-w-2xl w-full rounded-xl p-6 sm:p-8 text-slate-900 shadow-md space-y-6 relative text-left my-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 bg-slate-100 border border-slate-200 rounded-xl text-xs text-slate-900 focus:outline-none focus:border-orange-500"
                />
              </div>
              ...
              <div className="space-y-2 pt-2 border-t border-slate-200">
                  ...
                  <div className="flex items-center gap-1 bg-slate-50 p-1 rounded-xl border border-slate-200 text-[11px]">
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-lg font-bold transition-all cursor-pointer ${
                        imageUploadType === 'upload' ? 'bg-orange-600 text-white' : 'text-slate-400 hover:text-slate-900'
                      }`}
                    >
                    ...
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-lg font-bold transition-all cursor-pointer ${
                        imageUploadType === 'url' ? 'bg-orange-600 text-white' : 'text-slate-400 hover:text-slate-900'
                      }`}
                    >
                    ...
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-lg font-bold transition-all cursor-pointer ${
                        imageUploadType === 'preset' ? 'bg-orange-600 text-white' : 'text-slate-400 hover:text-slate-900'
                      }`}
                    >
                ...
                {imageUploadType === 'upload' && (
                  <div
                    ...
                    className="border-2 border-dashed border-slate-200 hover:border-orange-500/60 bg-slate-50/60 rounded-xl p-4 text-center cursor-pointer transition-colors space-y-1"
                  >
                    <input
                      ref={fileInputRef}
                      type="file"
                ...
                {imageUploadType === 'url' && (
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 bg-slate-100 border border-slate-200 rounded-xl text-xs text-slate-900 font-mono focus:outline-none focus:border-orange-500"
                  />
                )}
                ...
                {imagePreview && (
                  <div className="relative h-32 rounded-xl overflow-hidden border border-slate-200 bg-slate-50">
                    ...
                    <div className="absolute top-2 left-2 bg-slate-50/80 px-2 py-0.5 rounded-md text-[10px] font-bold text-emerald-600">
                      ✓ Image Ready
                    </div>
                  </div>
                )}
              ...
              <div className="space-y-2 pt-2 border-t border-slate-200">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 px-3.5 py-2 bg-slate-100 border border-slate-200 rounded-xl text-xs text-slate-900 focus:outline-none focus:border-orange-500"
                  />
                  <button
                    type="button"
                    onClick={handleAddInclude}
                    className="px-3.5 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-900 text-xs font-bold border border-slate-200 transition-colors cursor-pointer"
                  >
                    Add
                  </button>
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 max-h-32 overflow-y-auto pt-1">
                  {formData.includes.map((item, index) => (
                    <div
                      key={index}
                      className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-slate-50 border border-slate-200 text-xs text-slate-700"
                    >
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                />
                <label htmlFor="packageActiveCheckbox" className="text-xs text-slate-700 font-semibold cursor-pointer">
              ...
              <div className="flex justify-end gap-2.5 pt-4 border-t border-slate-200">
                <button
                  type="button"
                  onClick={() => setIsModalOpen(false)}
                  className="px-4 py-2.5 rounded-xl border border-slate-200 text-slate-700 hover:text-slate-900 text-xs font-semibold"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white text-xs font-extrabold shadow-sm shadow-orange-500/20 disabled:opacity-50 cursor-pointer"
                >
