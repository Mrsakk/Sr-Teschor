  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-white tracking-tight">Business & Partner Management</h2>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 shadow-lg flex flex-col md:flex-row items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2.5 w-full md:w-auto">
          <select
            ...
            className="px-3 py-2 bg-slate-800 border border-slate-700 rounded-xl text-xs text-slate-200 focus:outline-none focus:border-emerald-500 max-w-[140px]"
          >
                ...
                businesses.map((biz) => {
                  ...
                  return (
                    <tr key={biz.id} className="hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="px-5 py-3.5">
                        <div>
                          <p className="font-semibold text-slate-200">{biz.owner?.name || 'N/A'}</p>
                      ...
                      <td className="px-5 py-3.5 text-slate-300">
                        <div className="flex items-center gap-1 font-semibold text-amber-400">
                          ...
                          <span className="text-slate-500 text-[10px]">({biz.review_count || 0})</span>
                        </div>
                      ...
                      <td className="px-5 py-3.5 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          <button
                            ...
                            className="px-2.5 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-white font-medium text-xs flex items-center gap-1"
      ...
      {isReviewOpen && selectedBiz && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-2xl w-full max-h-[85vh] overflow-y-auto p-6 shadow-2xl relative text-left">
            <button
            ...
            <div className="py-4 space-y-4 text-xs text-slate-300">
              <div>
                <p className="font-bold text-white uppercase text-[11px] mb-1">Description</p>
                <p className="text-slate-300 leading-relaxed">{selectedBiz.description || selectedBiz.short_description}</p>
              </div>

              <div className="grid grid-cols-2 gap-3 bg-slate-800/50 p-3 rounded-2xl border border-slate-800">
                <div>
                  <span className="text-slate-400 block text-[10px] uppercase font-bold">Contact Phone</span>
                  <span className="font-semibold text-white">{selectedBiz.phone || 'N/A'}</span>
                </div>
                <div>
                  <span className="text-slate-400 block text-[10px] uppercase font-bold">Official Website</span>
                  <span className="font-semibold text-emerald-400">{selectedBiz.website || 'N/A'}</span>
                </div>
              </div>

              {/* Admin Feedback / Approval Notes */}
              <div>
                <label className="block font-bold text-white uppercase text-[11px] mb-1.5">
                ...
                <textarea
                  ...
                  className="w-full p-3 bg-slate-800 border border-slate-700 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500"
                />
              </div>
            </div>
